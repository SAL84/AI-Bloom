...
const evM3: CourseModule = {
  ...
  quiz: [
    { q: 'What is the most defensible use of public benchmark scores?', options: ['Selecting the production model for a specific workload without further testing', 'Coarse shortlisting and tracking capability trends, with private evals making the decision', 'Setting the acceptance criteria written into a customer contract', 'Estimating cost per task once the workload runs at production volume'], correct: 1 },
    { q: 'Why does benchmark contamination inflate scores even when the test set was never directly included in training?', options: ['Because benchmarks are graded by model judges that reward the phrasing they were trained on most', 'Because larger models memorise more of their training corpus by design', 'Because solutions, discussions and derivatives of popular benchmarks pervade training corpora', 'Because contamination only affects multiple-choice formats, and that is what most benchmarks use'], correct: 2 },
    { q: 'Which assertion style makes a regression suite most likely to survive a model upgrade?', options: ['Exact string match against a reference answer stored alongside each case in the suite', 'Checking that every response falls within a fixed word-count band', 'Requiring list items to appear in the same order as the reference', 'Asserting required facts, schema validity, and post-conditions on the world'], correct: 3 },
    { q: 'Why separate a prompt\'s task contract from its model-specific tuning?', options: ['So the suite tests durable behaviour while tuning is re-fitted per model', 'To reduce the token count of the assembled system prompt sent on every call', 'Because providers require prompts to be submitted in structured, labelled sections', 'To let the judge grade the task contract and the tuning layer as separate scores'], correct: 0 },
    { q: 'What is the argument against hard-gating merges on small aggregate eval movements?', options: ['Aggregate scores cannot be computed reliably inside a continuous integration pipeline', 'Gates that fire on statistical noise get disabled, after which nothing is gated', 'Small aggregate movements are always traceable to flaky infrastructure', 'Merge gates cannot be combined with a nightly suite'], correct: 1 },
    { q: 'Given a fixed eval budget, what generally yields more information?', options: ['More runs per existing case, since run-to-run variance is the main problem', 'More cases, since case diversity samples the input distribution', 'A larger judge model applied to the same set of cases', 'Longer timeouts per run so fewer cases fail on infrastructure'], correct: 1 },
    { q: 'Your offline eval scores stay flat while production quality complaints rise. What is the most likely explanation?', options: ['The judge model has been left at too low a temperature', 'Production is running with a different random seed to the suite', 'The eval set no longer represents current traffic, or has been tuned against', 'Tracing overhead is degrading the quality of production responses'], correct: 2 },
    { q: 'Why is cost per successful task a better metric than cost per API call for agents?', options: ['Because providers bill agent workloads on a different tariff to single calls', 'Because it includes retries and every loop iteration a per-call figure hides', 'Because per-call pricing changes too frequently to track across model providers', 'Because latency and cost per task are ultimately measurements of the same thing'], correct: 1 },
    { q: 'Why is a mean score the wrong way to report a safety eval?', options: ['Because safety graders cannot produce numeric scores, only pass and fail labels', 'Because safety sets contain fewer cases than quality sets', 'Because the helpfulness rubric already catches harmful outputs', 'Because safety is a floor — one harmful output can be the whole incident'], correct: 3 },
  ],
};
