...
const agM5: CourseModule = {
  ...
  quiz: [
    { q: 'Why is approval fatigue described as rational rather than as a discipline problem?', options: ['Because approvers are rarely trained on what the agent is actually permitted to do', 'Because a queue with a low base rate of problems makes close reading a poor use of attention', 'Because approval interfaces load slowly and interrupt whatever else the approver was in the middle of', 'Because approvals are usually delegated to the most junior person available on the rota'], correct: 1 },
    { q: 'What should happen when an approval request times out with no response?', options: ['The action proceeds by default, since blocking the queue causes more harm than it prevents', 'The run fails immediately and its state is discarded to avoid holding stale work', 'The action is abandoned or held with the run state preserved so the decision can still be made', 'The request is re-sent automatically at intervals until somebody finally answers it'], correct: 2 },
    { q: 'An agent has an escalation tool but its escalation rate is zero. What is the most likely explanation?', options: ['The tasks have genuinely never been ambiguous enough to require a human decision', 'The escape hatch is unreachable in practice, or metrics count escalation as a failure', 'The model is performing above expectations and resolving the ambiguity correctly on its own', 'The escalation schema demands too many required fields for the model to fill in reliably'], correct: 1 },
    { q: 'Where should the majority of escalation triggers live?', options: ['In the runtime, as structural conditions such as scope violations and budget limits', 'In the model\'s own assessment of its uncertainty, expressed as a confidence threshold', 'In the approval interface, where a human reviewer can judge each request on its merits', 'In the tool descriptions, so the model sees the escalation rule as it selects an action'], correct: 0 },
    { q: 'What is the correct status of an agent\'s generated explanation of why it acted?', options: ['A reliable account of the process the model actually followed, reconstructed from its own reasoning', 'An audit record sufficient on its own, provided it is timestamped', 'A summary whose accuracy a careful reader can confirm from the text', 'Generated text that may be accurate or a plausible reconstruction, to be shown beside the trace'], correct: 3 },
    { q: 'Autonomy is being widened for an agent. Which piece of evidence is most directly relevant to removing a specific approval gate?', options: ['The total tokens consumed per run before and after the gate was introduced', 'Overall user satisfaction with the product since the gate has been in place', 'The approval rate and decision latency on that gate, plus whether it was actually being read', 'The number of tools registered behind that gate and how often each is called'], correct: 2 },
    { q: 'What is the honest limit of human oversight once an agent takes thousands of actions per day?', options: ['People can review everything, provided the interface presents each action efficiently enough to scan', 'People can sample, handle exceptions and investigate incidents, but cannot assure every action', 'Oversight becomes unnecessary once measured failure rates drop below an agreed threshold', 'Review can be delegated entirely to a second model acting as an independent boundary on actions'], correct: 1 },
    { q: 'A prohibition list entry has no corresponding enforcement point in the runtime. What is the appropriate response?', options: ['Record the gap and define a detection with a response, not a prompt instruction', 'Remove the entry, since a rule nobody can enforce only creates false confidence in the list', 'Move the entry into the system prompt and treat the prohibition as handled from then on', 'Escalate every action of that type to a human until enforcement can be built into the runtime'], correct: 0 },
  ],
};
