...
const aiEssentials: Course = {
  ...
  modules: [
    // ── M1: AI Basics & Journey ───────────────────────────────────────────
    {
      ...
      quiz: [
        { q: 'What is the most accurate way to describe AI in everyday life in 2026?', options: ['A future technology that most people will adopt within the next few years', 'A passing trend driven by marketing budgets', 'A specialist tool that only engineers and researchers have reason to use', 'A substrate already underneath the tools people use daily'], correct: 3 },
        ...m1.quiz,
      ],
    },
    ...
    // ── M2: AI Frontiers ──────────────────────────────────────────────────
    {
      ...
      quiz: [
        { q: 'What does "multimodal AI" mean?', options: ['AI that processes multiple data types such as text, image, and audio', 'AI distributed across multiple servers for higher throughput', 'AI that can switch between several distinct conversational personality modes', 'A prompting technique that chains several steps together'], correct: 0 },
        { q: 'When would you choose a standalone computer vision model over a multimodal LLM?', options: ['For high-volume, single-task visual classification at low cost and latency', 'When you need language reasoning about what appears in the image', 'When you want a written explanation alongside each detection', 'For zero-shot generalisation to visual categories that were never seen in training'], correct: 0 },
        { q: 'Which attack type requires multimodal AI to detect and cannot be caught by text-only filters?', options: ['SQL injection through a web form field', 'Password spraying against a login endpoint', 'Visual phishing using spoofed logo images in screenshots', 'Credential stuffing using username lists from earlier breaches'], correct: 2 },
        { q: 'In a high-volume production NLP pipeline, why might a small fine-tuned classifier beat calling an LLM API?', options: ['A general LLM cannot be constrained to a fixed label set reliably enough for production classification', 'Small specialist models are typically far cheaper and lower-latency than LLM API calls at scale', 'LLM APIs do not support batching, so throughput cannot be scaled up', 'Small fine-tuned models are always more accurate than a general model on any task'], correct: 1 },
        { q: 'What does a Vision-Language-Action (VLA) model do that earlier robot controllers did not?', options: ['It compresses the robot\'s camera video so far longer recordings fit in on-board storage', 'It moves control from the GPU onto a general-purpose CPU, cutting the robot\'s power draw and heat output substantially', 'It takes a natural-language goal plus visual input and outputs robot actions, generalising across tasks', 'It runs entirely in simulation, so a physical robot is never required at deployment time'], correct: 2 },
        { q: 'Which is the most honest reason to pick one AI frontier over another?', options: ['It pays the highest salaries in the current hiring market', 'It is the frontier mentioned most often in social media and industry press', 'People you already know are working in that field', 'Your existing background gives you the largest unfair advantage there'], correct: 3 },
        { q: 'Why is voice the hardest modality to ship well in production AI?', options: ['Recognition accuracy remains far below the level real users tolerate, so transcripts are unreliable', 'Voice models cannot be served from the cloud at all and must run entirely on the user\'s device, which limits quality', 'Users prefer typing in nearly every situation, so voice interfaces go unused even when they work well', 'The whole pipeline — recognition, inference, and synthesis — must fit a roughly one-second latency budget to feel natural'], correct: 3 },
      ],
    },
    ...
    // ── M4: AI Ethics & Safety ────────────────────────────────────────────
    {
      ...
      quiz: [
        { q: 'What is "representation bias" in AI?', options: ['Skew in how AI systems are portrayed across news coverage and popular media', 'When a group is underrepresented in training data, so the model performs worse for them', 'A numerical error in the model\'s internal vector representations of concepts', 'Bias introduced later by prompt-engineering choices rather than by the underlying training data'], correct: 1 },
        ...
        { q: 'What is "Goodhart\'s Law" and why does it matter for AI?', options: ['Models always converge on the correct answer given enough training data, so any remaining error is purely a data-volume problem', 'AI models cannot generalise beyond their training distribution, so novel inputs always fail', 'When a measure becomes a target it stops being a good measure — AI systems find unexpected ways to maximise the reward', 'Interpretability is impossible for large neural networks, so oversight has to be purely behavioural'], correct: 2 },
        { q: 'What is a known limit of chain-of-thought output as an explainability tool?', options: ['It only works for text models and cannot be applied to multimodal or vision systems', 'It makes responses shorter and less detailed, which hides the intermediate steps from the person reviewing them', 'The written reasoning does not always faithfully reflect the computation that produced the answer', 'It is prohibited for high-risk systems under the EU AI Act\'s transparency provisions'], correct: 2 },
        { q: 'What is the "accountability gap" in AI?', options: ['Responsibility for harm fragments between developer, deployer, and user — each points at another, so no one answers', 'AI systems cannot keep audit logs detailed enough for anyone to reconstruct how a given decision was reached', 'Regulators lack the legal authority to hold AI companies to account for harm caused downstream by their systems', 'AI decisions happen too fast for any human to review before they take effect'], correct: 0 },
      ],
    },
    ...
    // ── M5: What is Next in AI ────────────────────────────────────────────
    {
      ...
      quiz: [
        { q: 'What distinguishes a reasoning model from a standard LLM?', options: ['It generates an explicit chain-of-thought scratchpad at inference time before answering', 'It has a substantially larger context window, so it can hold the whole problem at once', 'It is trained on far more data, including large specialised corpora of mathematics and code', 'It uses retrieval augmentation by default, grounding every answer in retrieved source documents'], correct: 0 },
        { q: 'What is the defining characteristic of an AI agent versus a copilot?', options: ['Agents run on larger models than copilots, which is what allows them to act independently', 'Agents operate with no human oversight at any point, whereas a copilot always keeps a human reviewer in the loop', 'Agents are always hosted in the cloud, while a copilot runs locally inside the application', 'Agents autonomously pursue a goal across multiple steps using tools, rather than assisting a human on one task'], correct: 3 },
        ...
        { q: 'What is the key privacy advantage of on-device AI models?', options: ['They are more accurate than cloud models of the same family', 'They are free to use, since there is no per-request billing to a provider', 'Data is processed locally and never sent to a third-party server', 'They need no configuration or setup before first use'], correct: 2 },
      ],
    },
  ],
};
