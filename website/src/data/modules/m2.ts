...
const m2: CourseModule = {
  ...
  quiz: [
    { q: 'A team wants an AI assistant that answers questions using their internal documents. What is the right architectural answer?', options: ['Use RAG to retrieve relevant document sections at query time', 'Fine-tune a custom model on the document corpus so it memorises the content', 'Train a model from scratch on the company\'s own documents', 'Paste the whole document set into a larger context window on every question'], correct: 0 },
    { q: 'Why might a 2M-token context window not solve "analyse our entire knowledge base"?', options: ['Two million tokens exceeds what most companies have', 'Models often degrade attention mid-context (lost in the middle)', 'Context windows that large are not technically possible on current hardware', 'Tokens are only an approximation, so document length cannot be measured reliably'], correct: 1 },
    { q: 'A hallucination is best described as:', options: ['The model deliberately lying to avoid admitting that it does not know the answer', 'A software bug in the serving code that corrupts the output', 'Fluent output not grounded in reality — probable tokens, not true facts', 'A jailbreak attempt that has bypassed the model\'s safety training'], correct: 2 },
    { q: 'Someone says "we need to fine-tune the AI on our company data." The most likely honest diagnosis is:', options: ['They genuinely need fine-tuning, since only training can make the model aware of their data', 'Fine-tuning is the right default whenever the data is proprietary', 'They need RLHF run against their own environment to align the model to their workflows', 'It is a RAG request in disguise — they need knowledge injection, not behaviour change'], correct: 3 },
    { q: 'What does low temperature (near 0) mean for a classification task?', options: ['The model returns answers faster because it explores fewer candidate tokens', 'Responses become more creative, which helps the model cover unusual edge-case categories', 'The model consumes less GPU memory per request, lowering the cost of the batch', 'Outputs are maximally deterministic — the model always picks the most probable token'], correct: 3 },
    { q: 'Embeddings let a system spot a scam email it has never seen before because:', options: ['They match against the exact keyword strings found in previously reported scam emails', 'Semantically similar concepts cluster together in vector space regardless of exact wording', 'They retrain the model on each new scam pattern as it is reported', 'They widen the context window so the whole message thread fits at once'], correct: 1 },
  ],
};
