...
const evM2: CourseModule = {
  ...
  quiz: [
    { q: 'Why is failure-weighted sampling of production traffic especially valuable when building an eval set?', options: ['It is the cheapest form of sampling to implement', 'It guarantees an unbiased estimate of overall system quality', 'Sessions with retries, rephrasings, or escalations concentrate the highest-value cases', 'It removes the need for privacy review'], correct: 2 },
    { q: 'What is the main limitation of a mostly synthetic eval set?', options: ['Synthetic cases are too expensive to generate at scale', 'It inherits the assumptions and style of whatever generated it, so it cannot reveal what users actually ask', 'Synthetic cases cannot be graded automatically', 'Model judges refuse to grade synthetic inputs'], correct: 1 },
    { q: 'Why keep a held-out eval set separate from the set you iterate against?', options: ['To reduce the compute cost of running evals', 'To satisfy data-retention requirements', 'Because held-out sets can be graded without a rubric', 'Because the gap between the two scores measures how much recent progress was overfitting'], correct: 3 },
    { q: 'A case that every candidate system fails is most often a sign of what?', options: ['A mislabelled or broken case that should be re-verified', 'A genuinely hard task worth prioritising', 'An overly capable judge model', 'Insufficient runs per case'], correct: 0 },
    { q: 'Why are several independent binary criteria usually preferable to one five-point quality scale?', options: ['Binary criteria need no rubric', 'Five-point scales cannot be applied by model judges', 'Rater agreement collapses on mid-scale distinctions, while binary checks are applied consistently and diagnose what failed', 'Binary criteria always produce higher scores'], correct: 2 },
    { q: 'What is the standard mitigation for position bias in pairwise LLM-as-judge comparisons?', options: ['Increase the judge\'s temperature to randomise its choice', 'Run both orderings and treat order-dependent verdicts as ties', 'Truncate both responses to the same length', 'Use a larger judge model'], correct: 1 },
    { q: 'What does it mean to calibrate a model judge properly?', options: ['Tune its temperature until scores stabilise across runs', 'Ask it to explain each score in detail before assigning it', 'Compare its labels against human labels as a confusion matrix and benchmark agreement against the human-human ceiling', 'Run it three times per case and take the majority verdict'], correct: 2 },
    { q: 'You measure 80% on 30 cases, change a prompt, and measure 85%. What is the honest conclusion?', options: ['A five-point improvement, safe to ship', 'The result is within the noise for a sample this small — too close to call', 'The prompt change caused a regression in an unmeasured slice', 'The eval set must be replaced'], correct: 1 },
  ],
};
