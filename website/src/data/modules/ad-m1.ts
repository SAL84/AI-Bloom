...
const adM1: CourseModule = {
  ...
  quiz: [
    { q: 'Why did symbolic AI ultimately fail to scale?', options: ['The hardware of that era was too slow to evaluate large rule bases in reasonable time', 'Rule-based decisions could not be explained to regulators or auditors, so compliance requirements blocked them from production deployment', 'Every capability had to be hand-authored, so coverage scaled with headcount while real-world edge cases scaled combinatorially', 'Neural networks were proven mathematically superior in the 1960s, so funding and talent left the field'], correct: 2 },
    { q: 'What did the Chinchilla result establish about compute-optimal training?', options: ['Models of the time were undertrained — optimal training uses roughly 20 tokens per parameter', 'Parameter count dominates — past a modest fixed data budget, extra tokens buy no further capability', 'Data quality stops mattering above a threshold, so raw token volume is the only variable worth tuning', 'Inference cost is independent of model size, so capacity planning only has to model the training run'], correct: 0 },
    { q: 'What defines the "second scaling era"?', options: ['Training exclusively on synthetic data generated by earlier models, removing any need for human text', 'Ever-larger pre-training runs, each generation adding an order of magnitude more parameters', 'Abandoning transformers for state-space architectures that scale more cheaply', 'Scaling inference-time compute — longer reasoning and search per request improve capability'], correct: 3 },
    { q: 'Why does memory bandwidth, not raw FLOPS, dominate LLM inference performance?', options: ['Accelerators spend most of their cycles idle, waiting on scheduler overhead between kernel launches', 'Generating each token streams the model weights and KV cache through the chip, so bandwidth sets tokens per second', 'Round-trip network latency between the client and the datacentre dominates, so on-chip bandwidth has little effect on throughput', 'Decoding is bottlenecked on the softmax, a serial step the GPU cannot parallelise across the vocabulary'], correct: 1 },
    { q: 'A team needs a churn-prediction model on structured tabular data with strict explainability requirements. The strongest default is:', options: ['Gradient-boosted trees — competitive accuracy, cheap, and far easier to explain', 'A frontier LLM called through an API, prompted with each customer record as text', 'A transformer trained from scratch on the tabular features for state-of-the-art accuracy', 'A multi-agent LLM system where one agent scores the risk and another writes the justification'], correct: 0 },
    { q: 'What structurally separates the current AI era from previous waves?', options: ['Public funding has replaced private investment as the main driver of frontier research budgets', 'Compute, data, architecture, and forecastable scaling inflected together, plus a revenue flywheel earlier waves never closed', 'The underlying algorithms are genuinely new — nothing in this wave traces back to the neural network research of earlier decades', 'Accelerator hardware became cheap enough that compute cost no longer constrains deployment'], correct: 1 },
    { q: 'Why has aggregate inference spend overtaken training spend across the industry?', options: ['Training costs have fallen sharply as accelerator efficiency improved, shrinking that side of the ledger to a rounding error', 'Frontier labs have largely stopped pre-training new base models and now only post-train existing ones', 'Models are trained once but queried at massive scale, and reasoning tokens plus agent loops multiply per-request usage', 'Regulators now cap the compute a single training run may consume, pushing budgets to the serving side'], correct: 2 },
    { q: 'When does self-hosting an open-weight model tend to beat using a frontier API?', options: ['Almost always, since open weights remove per-token fees and the hardware pays for itself quickly', 'For spiky, low-volume workloads with no data-residency constraints, where idle capacity is cheap to leave running', 'Rarely, because provider APIs are priced below the true cost of serving and self-hosting cannot match them', 'For steady high-volume, latency-sensitive, or data-sovereign workloads where infrastructure cost amortises'], correct: 3 },
  ],
};
