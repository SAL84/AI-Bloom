...
const secM2: CourseModule = {
  ...
  quiz: [
    { q: 'Why can a language model not reliably separate instructions from data?', options: ['Because most providers disable the separation feature by default for cost reasons', 'Because it all arrives as one token sequence and role is inferred, not parsed', 'Because a system prompt is always shorter than the user input it has to govern', 'Because tokenisation discards the punctuation that would mark where a quoted block ends'], correct: 1 },
    { q: 'What is the strongest reason input filtering cannot serve as a trust boundary for prompt injection?', options: ['Filters add latency that is unacceptable on an interactive request path', 'Most filters are proprietary, so their coverage cannot be audited or reasoned about', 'The space of ways to express an instruction is unbounded, and attackers adapt to whatever passes', 'Filters only work on text, while most injection now arrives through images and audio'], correct: 2 },
    { q: 'A poisoned tool description most directly exploits which fact?', options: ['Tool descriptions are executed as code by the runtime when the tool is registered', 'Tool names, descriptions and parameter docs enter the model\'s context as readable text', 'Descriptions are cached indefinitely by the client and never re-validated against the server', 'The protocol transmits tool descriptions in clear text, where they can be rewritten in transit'], correct: 1 },
    { q: 'Which property makes indirect injection hard to catch with session-level anomaly detection?', options: ['It only works against multimodal models, whose image channel the detector does not inspect', 'It requires valid credentials, so the session looks like ordinary authorised use', 'It always produces malformed input, which anomaly detectors are tuned to ignore as ordinary noise', 'Planting and activation are separated in time, so no attacker activity coincides with the action'], correct: 3 },
    { q: 'A component passes the lethal trifecta test — it has no outbound channel. What risk remains unaddressed?', options: ['Destructive and financial actions, and harm from confidently wrong output', 'None — removing one leg of the trifecta resolves the component\'s risk entirely', 'Only latency and cost concerns remain once the outbound channel is gone', 'The component becomes more vulnerable to direct injection, having lost a monitoring path'], correct: 0 },
    { q: 'Why is memory contamination more serious than a session-scoped injection?', options: ['Memory writes are far more expensive to store than session context', 'It prevents the agent from calling tools until the contaminated entry is cleared', 'It persists after the hostile content is gone and re-enters context looking trusted', 'It only affects multi-tenant systems, where one user\'s memory can be read by another'], correct: 2 },
    { q: 'Which of these is the correct decomposition of excessive agency?', options: ['Excessive latency, excessive cost, and excessive token consumption per run', 'Excessive training data, excessive fine-tuning, and insufficient evaluation', 'Excessive functionality, excessive permission, and excessive autonomy', 'Direct injection, indirect injection, and multimodal injection of instructions'], correct: 2 },
    { q: 'A team purges a poisoned document from the retrieval corpus but the agent still behaves oddly on that topic. What is the most likely explanation?', options: ['Derived artefacts — embeddings, caches, summaries, memory entries — were not cleared by the purge', 'The model memorised the document during training and reproduces it from weights', 'The corpus index is encrypted, so the purge could not match the document to remove it', 'Purging only takes effect after the serving model is restarted and the index reloaded'], correct: 0 },
    { q: 'Why organise jailbreak testing around mechanism categories rather than a maintained library of known payloads?', options: ['Category names present better to executives than a list of technical payload strings', 'Payload libraries cannot be run inside a continuous integration pipeline without approval', 'Specific strings get patched and decay into a test of what is already blocked, while mechanisms persist', 'Working with categories removes the need to test the deployed guardrail stack separately from the model itself'], correct: 2 },
  ],
};
