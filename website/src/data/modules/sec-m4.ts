...
const secM4: CourseModule = {
  ...
  quiz: [
    { q: 'What is the non-negotiable precondition for any adversarial testing described in this module?', options: ['A validated model judge for deciding whether each recorded attempt actually succeeded', 'Written authorisation and an agreed scope covering only systems you may test', 'A minimum number of documented attempts in each technique category before reporting', 'Access to the model provider\'s training data and its safety fine-tuning corpus'], correct: 1 },
    { q: 'Why use a benign marker or no-op tool as the objective in an injection test?', options: ['It runs faster than a realistic attempt, so more variants fit in the same test window', 'It avoids the need for written authorisation, since nothing harmful is attempted', 'It demonstrates the path without exercising the harm, and is safe to automate', 'It guarantees the model will comply, since the requested action is harmless'], correct: 2 },
    { q: 'Which assertion is most durable in a security regression test for an agent?', options: ['That the trace contains no privileged tool call following ingestion of external content', 'That the response text matches the expected refusal string word for word', 'That the run completed within its token budget and step ceiling', 'That the model returned a refusal message naming the policy it applied'], correct: 0 },
    { q: 'A defence is tuned until the automated adversarial suite reports no successes. What has most likely been achieved?', options: ['Robustness against adaptive attackers across the whole injection surface', 'Elimination of the need for manual red-team testing on that surface', 'A success judge validated against human labels on the same set of recorded attempts', 'A defence tuned to that suite, which an adapting attacker is not constrained by'], correct: 3 },
    { q: 'Why do conventional detection approaches often miss an AI incident?', options: ['Because model API traffic is encrypted end to end and cannot be inspected in transit', 'Because AI systems generate too little telemetry for a detection rule to fire on', 'Because the malicious activity happens off-network, inside the model provider\'s own inference stack', 'Because every call is authenticated, authorised and successful — the harm is in permitted actions'], correct: 3 },
    { q: 'When scoping an incident caused by poisoned retrieved content, what is the standard error?', options: ['Limiting the search window to when the symptom appeared rather than when the content was planted', 'Searching traces by run identifier rather than by the document that was retrieved', 'Including runs that produced no visible harm, which inflates the affected population unnecessarily', 'Quarantining the source document before its full retrieval history has been captured from the traces'], correct: 0 },
    { q: 'A team purges the poisoned source document and upgrades to a newer model family. Why is this insufficient eradication?', options: ['Newer model families are more susceptible to injection than the ones they replace', 'Purging a source always requires the whole corpus to be reindexed from scratch', 'Derived artefacts may survive and the capability path that allowed the harm remains open', 'A model upgrade invalidates the existing traces needed to confirm eradication'], correct: 2 },
    { q: 'Which of these is a posture metric rather than an activity metric?', options: ['Number of adversarial attempts run against production features over the last quarter', 'Number of requests blocked by the guardrail layer during the last reporting period', 'Proportion of AI features with a current threat model and a permission inventory', 'Number of AI security policies published and circulated across the engineering org'], correct: 2 },
  ],
};
