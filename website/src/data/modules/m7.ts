...
const m7: CourseModule = {
  ...
  quiz: [
    { q: 'A vendor answers "what model is under the hood?" with "we use cutting-edge proprietary AI." The best read of this answer is:', options: ['Proprietary models are always superior, so this is reassuring', 'The model choice does not matter as long as the demo works', 'The vagueness is diagnostic — press for the actual architecture', 'The vendor is protecting trade secrets, which is a positive maturity signal'], correct: 2 },
    { q: 'During a demo, the vendor declines to run one of your sanitised alerts, citing weeks of "environment preparation" needed first. This most likely signals:', options: ['A preview of integration cost — it needs curated data and parsers', 'Normal caution that says nothing about the product itself', 'Strong security discipline on the vendor side of the demo', 'That your alert data is too complex to be a fair test'], correct: 0 },
    { q: 'You ask a vendor about hallucination rates. Which answer indicates real engineering maturity?', options: ['"We use the latest frontier models available, which are extremely accurate"', '"Our guardrails prevent hallucinations entirely"', '"No customer has ever reported one to us"', '"Outputs are grounded and validated, and here is our measured error rate"'], correct: 3 },
    { q: 'A product is marketed primarily on cutting alert volume by 80%. The most important evaluation question is:', options: ['Whether the reduction could reach 90% with tuning', 'How the reduction is achieved — suppression, deduplication, or triage', 'Whether competitors claim bigger reductions in their marketing', 'How quickly the reduction appears after the tool is deployed'], correct: 1 },
    { q: 'Your team is furious with an incumbent tool, and a challenger demos brilliantly. The disciplined next step is:', options: ['Demand challenger evidence on your own data and cost the full migration', 'Move fast: analyst frustration is itself the strongest evaluation signal', 'Sign with the challenger but keep the incumbent licence as a backup indefinitely', 'Ask the incumbent to match the challenger demo before deciding anything'], correct: 0 },
    { q: 'A colleague wants to paste an active incident\'s evidence, including credentials found in a dump, into a consumer AI chatbot to speed up triage. The right call is:', options: ['Fine, since speed matters more than anything during a live incident', 'Fine, provided the whole chat history is deleted immediately afterwards', 'Acceptable for the credentials but not for the full incident timeline', 'Stop — credentials and live evidence never enter unapproved tools'], correct: 3 }
  ]
};
