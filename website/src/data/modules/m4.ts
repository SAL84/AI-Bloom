...
const m4: CourseModule = {
  ...
  quiz: [
    { q: 'What is the core shift when moving from a legacy SIEM to an agentic SOC platform?', options: ['A lower price per GB of telemetry ingested each month', 'More prebuilt connectors available out of the box', 'The cloud platform becomes the agent control plane, not a product swap', 'A faster query engine behind the same console'], correct: 2 },
    { q: 'In a modern SOC AI architecture, what sits above the classical ML detection layer?', options: ['GenAI for narrative, summarization, and reasoning over the ML signals', 'A second rules engine that runs the same correlation logic in parallel', 'A SOAR playbook that fires on every correlated alert', 'A compliance dashboard for the audit team to read'], correct: 0 },
    { q: 'A vendor claims "fully AI-powered detection." Best follow-up question:', options: ['How many parameters does the model behind the product actually have?', 'Which programming language is the detection engine written in?', 'How much GPU capacity does the product consume for each query?', 'Which detections are AI-driven, and how do you evaluate them?'], correct: 3 },
    { q: 'What makes NHI (non-human identity) a 2026 priority area in identity security?', options: ['Human accounts are now fully protected by the identity controls already deployed', 'Service accounts and API keys sit outside ITDR but enable lateral movement', 'NHI refers to a newly published compliance standard', 'All identity systems now use biometrics by default'], correct: 1 },
    { q: 'Where are autonomous AI agents producing the largest current wins in the SOC?', options: ['Strategic threat modeling for the whole enterprise', 'Replacing the judgement of security leadership', 'High-volume, low-judgment work like phishing triage and IOC enrichment', 'Running compliance audits end to end'], correct: 2 },
    { q: 'What changes most when AI is applied to threat hunting?', options: ['Threat actors stop attacking the environment', 'MITRE ATT&CK is replaced by a new framework', 'Hunting becomes fully automated, with no analyst input needed at any point in the loop', 'Query generation, result evaluation, and pivoting collapse from hours to minutes'], correct: 3 },
    { q: 'In a modern XDR, the GenAI layer typically:', options: ['Sits on top of classical ML, turning signal into narrative', 'Replaces every classical ML detection in the stack entirely', 'Performs the actual malware classification work all on its own', 'Is purely cosmetic and adds nothing of use to the console view'], correct: 0 },
    { q: 'Which three architectural controls answer the "our data is too sensitive" CISO objection?', options: ['SOC 2, ISO 27001, and GDPR compliance certificates', 'VPC Service Controls, Customer-Managed Encryption Keys, and data residency', 'Perimeter firewall, corporate VPN, and multi-factor authentication', 'API rate limiting, endpoint data-loss prevention, and a web application firewall'], correct: 1 }
  ]
};
