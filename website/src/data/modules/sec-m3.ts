...
const secM3: CourseModule = {
  ...
  quiz: [
    { q: 'Why is a purpose-built tool generally safer than a tool that accepts an arbitrary query?', options: ['It uses fewer context tokens, since its schema is far narrower than a general query interface', 'It cannot be steered into returning or affecting something other than its intended target', 'It executes faster, because the query plan is fixed rather than built at request time', 'It does not require a credential of its own, since it runs under the caller\'s identity instead'], correct: 1 },
    { q: 'An agent retrieves user-specific data using a shared service identity. What is the principal risk?', options: ['Higher inference cost per request, since the shared identity fetches more than is needed', 'The retrieval index becomes non-deterministic, returning a different ranking for each caller', 'It can return data the requesting user is not authorised to see — a confused-deputy disclosure', 'Traces become larger than necessary, because every run records the full corpus under one identity'], correct: 2 },
    { q: 'Why is default-deny egress described as the control that holds even when other layers fail?', options: ['Because models are trained to respect the network policy declared in their instructions', 'Because it blocks injected instructions before they ever reach the model\'s context window', 'Because it eliminates the need for approval gates on any outbound action the agent takes', 'Because it is enforced by infrastructure and therefore survives a fully persuaded model'], correct: 3 },
    { q: 'What does a sandbox specifically NOT protect against?', options: ['Arbitrary generated code reaching the host filesystem', 'Unbounded CPU and memory use by executed code', 'Actions taken through the agent\'s legitimate, approved tools outside the sandbox', 'Persistence of written state between runs'], correct: 2 },
    { q: 'What is the main reason to gate few actions rather than many?', options: ['Each gate adds latency that compounds across a multi-step run', 'Approval tooling is expensive to license per reviewer seat', 'Approvers cannot see the full tool parameters when volume is high, so they judge on the summary', 'High volume produces reflexive clicking, which documents a decision that never happened'], correct: 3 },
    { q: 'What does constrained decoding to a schema guarantee, and what does it not?', options: ['It guarantees valid form, not that the values chosen are correct or safe', 'It guarantees the model cannot be injected through a structured tool argument', 'It guarantees the call is authorised for the requesting user, since identity is in the schema', 'It guarantees that free-text fields within the schema contain no embedded instructions'], correct: 0 },
    { q: 'Genuine attacks are rare relative to total traffic. What follows for a guardrail classifier with a low false-positive rate?', options: ['The false-positive rate stops mattering once the base rate of attacks is low enough', 'It will block far more legitimate requests than attacks, pushing users toward unmonitored tools', 'It will fail to log true positives, which are drowned out by the volume of benign traffic', 'It will need retraining after every model upgrade to hold the same operating point'], correct: 1 },
    { q: 'Which trace field most often turns out to be missing when an incident needs reconstruction?', options: ['The prompt as actually assembled after templating, retrieval and context management', 'The HTTP status code returned by each model API call made during the run', 'The name of the model family that served the request and the version it was pinned to', 'The total token count consumed across the whole run, broken down by step and by tool'], correct: 0 },
  ],
};
