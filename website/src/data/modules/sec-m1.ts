...
const secM1: CourseModule = {
  ...
  quiz: [
    { q: 'Which assumption of classic application threat modelling does an LLM feature most directly violate?', options: ['That systems have identifiable owners', 'That instructions and data are separable and the separation is enforced by the interface', 'That attackers are financially motivated', 'That logs are retained for a fixed period'], correct: 1 },
    { q: 'Where should the trust boundary sit relative to the model in an LLM feature?', options: ['At the API edge, with everything after input validation treated as internal', 'Around the retrieval corpus only, since that is where attacker content arrives', 'Boundaries do not apply because the model is a managed service', 'Around the model, so its output is treated as untrusted by every consumer'], correct: 3 },
    { q: 'A threat model lists the model, prompts, corpus, tools and memory but omits outputs and humans. What is the practical consequence?', options: ['Nothing — those are not attacker-controllable', 'Output handling and approver manipulation go unmodelled, missing rendering-based exfiltration and misled approvals', 'The model becomes non-deterministic', 'Provenance tagging stops working'], correct: 1 },
    { q: 'Why rank attack-surface components by reach and reversibility?', options: ['Because irreversible actions have already affected a third party by the time you detect them, so they dominate real severity', 'Because reversible actions are more common', 'Because it matches the licensing model of most AI platforms', 'Because reach determines inference cost'], correct: 0 },
    { q: 'What is the main practitioner value of enumerating techniques with MITRE ATLAS?', options: ['It certifies a system as compliant', 'It provides working exploit code for each technique', 'It forces coverage past the biased sample you would produce from memory, and makes exclusions defensible', 'It replaces the need for a data-flow diagram'], correct: 2 },
    { q: 'A team marks an ATLAS technique as mitigated because a broadly related control exists elsewhere in the platform. What is wrong with this?', options: ['Nothing, provided the control is documented', 'ATLAS techniques cannot be mitigated individually', 'A related control is not a verified control for that technique — the matrix becomes reassurance rather than coverage', 'Mitigation status should be tracked per tactic, never per technique'], correct: 2 },
    { q: 'What belongs in an accepted-risk entry so it remains reviewable later?', options: ['The scenario, the reason for acceptance, a named accepter with a date, and what would reopen the decision', 'The CVE identifier and CVSS score', 'The vendor\'s security certifications', 'The number of red-team attempts made against it'], correct: 0 },
    { q: 'For prompt injection specifically, what is the honest way to represent it in a threat model?', options: ['As prevented, once an input filter is deployed', 'As out of scope, since it affects the model provider rather than the application', 'As resolved by using a newer model family', 'As expected to succeed, with containment limiting what it reaches and named detection signals'], correct: 3 },
  ],
};
