...
const evM4: CourseModule = {
  ...
  quiz: [
    { q: 'Why should a document-grounded system report retrieval quality and generation quality separately?', options: ['Because the two stages are usually built by different teams', 'Because retrieval metrics are cheaper to compute than end-to-end ones', 'Because a wrong answer has two different root causes — nothing relevant was found, or something relevant was found and misused — and one accuracy number cannot separate them', 'Because end-to-end accuracy cannot be measured once retrieval is involved'], correct: 2 },
    { q: 'What does recall@k measure in a retrieval evaluation?', options: ['The share of cases where at least one passage genuinely containing the answer appears in the top k results actually passed to the model', 'The proportion of retrieved passages that the model went on to cite', 'The average rank of the correct passage across all cases', 'The number of passages the search layer returns for each query'], correct: 0 },
    { q: 'An answer is fully supported by the passage it was given, but that passage is out of date. How should this be recorded?', options: ['As a retrieval failure, since the wrong passage was surfaced', 'As a judge calibration error', 'As an over-refusal', 'As grounded but incorrect — a corpus problem rather than a generation one'], correct: 3 },
    { q: 'An agent reaches the correct final state after several unnecessary tool calls and one destructive action that happened to be reversible. What does outcome-only grading do with this run?', options: ['Fails it, because the trajectory was wrong', 'Passes it, concealing a path that becomes an incident the first time the destructive action is not reversible', 'Flags it as non-deterministic and re-runs it', 'Cannot score it at all without a judge model'], correct: 1 },
    { q: 'Why are per-step assertions the highest-value addition to an agent eval suite?', options: ['They reduce the token cost of every run', 'They remove the need to grade the outcome', 'Late failures usually have early causes, so per-step checks fail the run where the premise broke rather than where the consequence appeared', 'They make agent runs deterministic'], correct: 2 },
    { q: 'Two reviewers frequently disagree when scoring the same outputs against the same criterion. What is the correct response?', options: ['Fix the rubric — replace vague adjectives with observable conditions and anchor each scale point with worked examples', 'Average their scores and carry on', 'Increase the sample size until the disagreement averages out', 'Replace both reviewers with a model judge'], correct: 0 },
    { q: 'A new version shows a strong lift in the first days of an online test, and the lift fades over the following weeks. What is the most likely explanation?', options: ['The guardrail thresholds were set too tightly', 'Randomisation was done at session level rather than user level', 'Contamination in the underlying model\'s training data', 'A novelty effect — engagement driven by the change being new, which decays as it stops being new'], correct: 3 },
    { q: 'Why must a system that routes easy cases to a cheaper model be evaluated end to end rather than component by component?', options: ['Because the smaller model cannot be measured on the same suite', 'Because a router that misclassifies difficulty can answer worse than the small model alone while costing more than the large one, having paid for both', 'Because routing changes which tokenizer applies at each stage', 'Because escalation rate can only be computed per component'], correct: 1 },
  ],
};
