...
const evM1: CourseModule = {
  ...
  quiz: [
    { q: 'What makes silent regressions the characteristic failure of vibes-based AI development?', options: ['The application crashes under load, but only for a small share of requests', 'Quality degrades without any error, exception, or failing build to signal it', 'They only appear when switching model providers, never within one family', 'They are caused by sampling temperature and disappear once it is lowered'], correct: 1 },
    { q: 'How does an eval assertion differ from a conventional unit-test assertion?', options: ['Evals assert exact string equality against a stored reference answer', 'Evals only run against production traffic and never inside a continuous integration pipeline', 'Evals assert properties and measure rates across runs, not one deterministic output', 'Evals require a model judge to grade every case, since the outputs are free-form text'], correct: 2 },
    { q: 'A case in your suite passes 70% of the time. What is the right response?', options: ['Loosen the grader until the case passes consistently on every run', 'Record the pass rate as a measurement and watch it for movement', 'Delete the case, since a non-deterministic result cannot be a regression signal', 'Re-run the case until it passes and record that run as the result'], correct: 1 },
    { q: 'What is the correct order when starting an eval practice?', options: ['Pick the standard industry metrics first, then collect data that fits them', 'Choose an eval platform first, then decide what it should measure for you', 'Read real traces and cluster failures, then build metrics for the biggest clusters', 'Write synthetic cases covering every feature in the product, then have a judge label them'], correct: 2 },
    { q: 'Why should a newly written eval case fail before you change the system?', options: ['Because a failing case is cheaper to run than a passing one', 'To confirm the grader actually tests the behaviour you intend to fix', 'Because the harness needs a failing baseline before it can compute a score', 'To satisfy the statistical significance requirements of the suite'], correct: 1 },
    { q: 'Your aggregate eval score improves after a change. What must you check before shipping?', options: ['That the run completed faster than the previous one on the same hardware', 'That the model version string was recorded alongside the aggregate score for the run', 'Nothing further, since a higher aggregate score is the definition of improvement', 'Per-slice results and guardrail metrics, in case a critical segment regressed'], correct: 3 },
    { q: 'Which practice best protects an eval suite against Goodhart-style overfitting?', options: ['Maintaining a held-out test set the team never tunes against, refreshed from production', 'Increasing the number of runs per case to average out sampling noise', 'Using a larger judge model that is harder to game with surface features', 'Reporting one aggregate score to leadership so the target stays stable'], correct: 0 },
  ],
};
