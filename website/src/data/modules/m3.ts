...
const m3: CourseModule = {
  ...
  quiz: [
    { q: 'What problem does MCP primarily solve?', options: ['N×M integration complexity between AI hosts and tools', 'The cost of training a model on proprietary internal data', 'Hallucination in tool-augmented responses', 'Unpredictable token pricing across providers'], correct: 0 },
    { q: 'A customer wants an "agent" that just summarizes alerts. The honest assessment:', options: ['Yes — it calls a model and produces a result, so it qualifies as an agent', 'Yes, provided it runs on a frontier-class reasoning model', 'The distinction is only marketing — call it whatever the customer already prefers to call it', 'It is likely an automated workflow, not really an agent — no goal-directed iteration'], correct: 3 },
    { q: 'For most enterprise security deployments, the right default autonomy level is:', options: ['Level 0 — a human approves every single step the agent takes', 'Level 1 — approve consequential steps, autonomous on routine', 'Level 3 — full autonomy, with review only after the fact', 'No autonomy at all — the agent only drafts suggestions for a human'], correct: 1 },
    { q: 'A2A is to ___ as MCP is to ___', options: ['agent-to-agent ; agent-to-tool', 'agent-to-tool ; agent-to-agent', 'training ; inference', 'They are the same protocol under two names'], correct: 0 },
    { q: 'When a prospect says "plugin," the right redirect is:', options: ['Agree and adopt their terminology to keep the conversation moving', 'Redirect to MCP — the standardized, security-conscious evolution of the plugin model', 'Tell them plugins are deprecated and that the whole concept no longer applies to modern systems', 'Ask which vendor\'s plugin framework they have in mind before answering'], correct: 1 },
    { q: 'A subagent should receive privileges by:', options: ['Inheriting the full privilege set of the orchestrator that spawned it', 'Reading whatever it needs from a shared credential store', 'Delegation — only the minimum required for its specific task', 'Prompting the user for additional privileges at runtime as it needs them'], correct: 2 },
    { q: 'A prospect asks "how do we enforce policy on our agents?" They are asking for:', options: ['A more capable model that follows instructions more reliably', 'A stricter system prompt that spells out exactly what the agent is forbidden to do', 'More infrastructure capacity so policy checks do not slow things down', 'Hooks — pre/post execution interceptors that validate, filter, approve, and log'], correct: 3 },
