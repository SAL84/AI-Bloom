  ...
  lessons: [
    ...
    },
    {
      ...
      slides: [
        ...
        { heading: 'When to Use Each', body: 'Use a skill when a capability is reused across multiple agents (avoid duplication) or when you need to control which agents can access it (governance). Use MCP/plugins when connecting to an external system — MCP for new builds, "plugin" as a legacy redirect term. Use a subagent when a task needs different data access, a different model, or isolated blast radius — a subagent failure should not take down the orchestrator. Use hooks when you need cross-cutting policy enforcement without modifying every agent individually: audit logging, input validation, approval gates, and rate limiting all belong in hooks, not in agent logic. If you find yourself writing the same validation or logging code in multiple agents, you should be writing a hook.' },
        { heading: 'Real-World Scenario: Phishing Alert Triage', body: 'A phishing alert fires in Google SecOps. Here is how the four concepts appear in one workflow: (1) The orchestrator agent receives the alert and spawns an IOC Enrichment subagent — scoped with read-only access to GTI and VirusTotal, nothing else. (2) The subagent calls two skills: gtl_lookup (checks the suspicious URL against Mandiant intel) and vt_scan (submits the file hash). Skills are self-describing — the subagent discovers and calls them without hardcoded API logic. (3) Before each skill call, an input hook strips injection payloads from the retrieved email body so poisoned content cannot manipulate the model. (4) The orchestrator synthesizes findings and recommends "isolate host." An action hook intercepts — pauses execution, sends an approval notification to the on-call analyst, and waits. Analyst clicks approve. Only then does the host isolation tool call fire. (5) An audit hook writes the full trace — every skill call, every model decision, the analyst approval timestamp — to an immutable log. That log is what the CISO sees in the Agent Security Dashboard. Each concept plays a distinct role; none of them is interchangeable with another.' }
      ]
    }
  ],
  quiz: [
