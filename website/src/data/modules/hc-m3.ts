...
const hcM3: CourseModule = {
  ...
  quiz: [
    { q: 'A study reports strong results using internal validation only. What has it demonstrated?', options: ['That the model generalises to other institutions\' data', 'That the model is ready for prospective deployment on the ward', 'That the model will transfer to a different make of scanner', 'That the model did not simply memorise its training data'], correct: 3 },
    { q: 'Why do models typically perform worse in prospective evaluation than in retrospective studies?', options: ['Real-time data is messier and includes the hard cases curation removes', 'Prospective studies rely on weaker statistical methods throughout', 'Models degrade physically the longer they stay installed', 'Clinicians deliberately introduce errors into the input'], correct: 0 },
    { q: 'A model has high AUC but poor calibration. What is the practical consequence?', options: ['It cannot rank patients by risk in the right order', 'It will run more slowly once it is in production', 'Its stated probabilities do not match real frequencies', 'Nothing practical follows, since AUC is the figure that matters'], correct: 2 },
    { q: 'The care-management algorithm that used cost as a proxy for need caused harm because:', options: ['The model was miscoded, so it scored every patient incorrectly', 'Spending is lower for Black patients at equal illness, so need was understated', 'The training dataset was too small to represent the population', 'It excluded cost data from the calculation entirely'], correct: 1 },
    { q: 'A vendor reports a single aggregate accuracy figure and no subgroup breakdown. You should conclude:', options: ['Performance must be equal across all patient groups', 'Subgroup analysis is not applicable to this kind of model', 'The aggregate figure is sufficient if it is high enough', 'There is no evidence of equitable performance'], correct: 3 },
    { q: 'Which is the strongest reason local validation cannot be skipped when external evidence is strong?', options: ['Regulators require the same paperwork to be filed twice at each site', 'External studies are usually fraudulent or heavily massaged', 'Prevalence, equipment, case mix, and data conventions are local', 'Local data sets are always larger than the external ones'], correct: 2 },
    { q: 'Which monitoring layer typically gives the earliest warning of drift?', options: ['Input monitoring — distributions and formats shift before outputs', 'Outcome monitoring, because it measures what actually matters most', 'The annual re-accreditation review of the deployed tool', 'Clinician satisfaction surveys gathered each quarter'], correct: 0 },
    { q: 'A deterioration model appears less accurate after deployment, while patient outcomes have improved. The most likely explanation is:', options: ['The model file itself has become corrupted on the hospital server', 'A feedback loop — intervention prevented the predicted outcomes', 'The monitoring system has been misconfigured since the tool went live', 'Clinicians quietly stopped using the tool at some point after launch'], correct: 1 },
  ],
};
