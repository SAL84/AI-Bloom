...
const agM4: CourseModule = {
  ...
  quiz: [
    { q: 'Why is lowering sampling randomness an inadequate answer to non-determinism in an agent?', options: ['It makes generation slower and noticeably degrades quality on open-ended tasks', 'Providers do not guarantee reproducible output, so correctness must rest on outcomes rather than on the path', 'It suppresses tool calling, because the model collapses onto the single most likely continuation of the prompt', 'It breaks prompt caching, since sampling settings form part of the cache key on most serving platforms'], correct: 1 },
    { q: 'Which trace field is most often missing and most often ends an investigation?', options: ['The HTTP status of each model call, without which failures cannot be told apart from refusals', 'The number of tools registered for the run, which sets how much of the context is fixed', 'The assembled prompt as actually sent, after templating, retrieval and compaction', 'The wall-clock start time of the run'], correct: 2 },
    { q: 'An agent repeatedly calls a tool name that does not exist. What is the most useful interpretation?', options: ['The model is too small for the task and should be replaced with a stronger one', 'The context window is too small, so the real tool definitions were truncated away', 'Tool validation is too strict and should be relaxed so the call can be attempted', 'The invented name is a specification for a capability the agent needed and was not given'], correct: 3 },
    { q: 'Which failure mode is described as quiet, surfacing later downstream as a data problem nobody attributes to the agent?', options: ['An infinite loop that burns the whole step budget on one repeated action', 'A hallucinated tool call naming a capability that does not exist', 'Context exhaustion that silently truncates the earliest instructions', 'Premature stop — success declared on partial work'], correct: 3 },
    { q: 'Why does agent cost grow superlinearly with run length in a naive loop?', options: ['Because every step re-sends the accumulated context, so input tokens dominate and compound', 'Because output tokens are priced higher than input tokens and a long run generates far more of them', 'Because each additional step spawns a subagent with a context window of its own', 'Because tool executions are billed per second of wall-clock time rather than per call'], correct: 0 },
    { q: 'A run exhausts its budget after processing sixty of a hundred records. What is the worst option?', options: ['Return the sixty with explicit coverage information and a resumable checkpoint', 'Return the sixty without any indication that the run was incomplete', 'Return nothing and escalate to a human with the full run state attached', 'Return the sixty and queue a follow-up run to complete the remaining forty'], correct: 1 },
    { q: 'What is the common failure mode shared by all fallback strategies?', options: ['They require a second model provider, which doubles the integration surface to maintain', 'They cannot be traced, because the fallback path bypasses the instrumented main path', 'Falling back silently, so the system degrades to a worse path while reporting ordinary success', 'They always increase cost, since the failed primary attempt is paid for before the retry'], correct: 2 },
    { q: 'Why should full-fidelity traces be access-controlled as sensitive data rather than as ordinary logs?', options: ['Because they are large and expensive to store at production trace volumes', 'Because an assembled prompt contains a copy of the retrieved documents, user data and tool results', 'Because they record which model version served each call, which is commercially sensitive information', 'Because ordinary observability tooling cannot enforce a retention limit on trace payloads of that size'], correct: 1 },
  ],
};
