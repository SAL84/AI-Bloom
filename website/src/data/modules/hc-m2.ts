...
const hcM2: CourseModule = {
  ...
  quiz: [
    { q: 'A vendor says their product "reads chest imaging with AI". What is the first question to ask?', options: ['How many parameters does the underlying neural network have?', 'Which finding, on which modality, validated in which population?', 'How many seconds does it take to process a single study end to end?', 'Which cloud region does the processing run in?'], correct: 1 },
    { q: 'A model with fixed sensitivity and specificity is moved from a specialist referral centre to general screening. What changes?', options: ['Its sensitivity falls automatically once the setting is lower-risk', 'Its specificity rises automatically once the setting is lower-risk', 'Nothing changes, because both metrics are fixed properties of the model', 'The proportion of positive flags that are true findings falls'], correct: 3 },
    { q: 'Which deployment mode best preserves the reader\'s independent judgement?', options: ['Concurrent display while the reader works through the study', 'Autonomous reading, with a sample audited periodically afterwards', 'Second-reader mode, with output revealed only after an independent read', 'Showing the model\'s output before the study is opened'], correct: 2 },
    { q: 'A model performs excellently at its development site and poorly at a new hospital, with no visible image quality difference. The most likely explanation is:', options: ['Distribution shift — different equipment, protocols, or patient mix', 'The new hospital\'s radiologists are less skilled at this finding', 'The model needs more parameters to handle the extra caseload', 'Network latency is degrading the quality of each inference'], correct: 0 },
    { q: 'Why is the omission form of automation bias considered more dangerous than the commission form?', options: ['It happens more often than the commission form in every study', 'It affects only readers who are new to the modality', 'It is easier for auditors to pick up in a retrospective review of flagged cases', 'An absent flag leaves no artefact for anyone to review afterwards'], correct: 3 },
    { q: 'A vendor publishes strong standalone model metrics. What do they tell you about reader-plus-model performance?', options: ['Combined performance will be at least as good as the model working alone', 'Very little — combined performance has to be measured directly', 'Combined performance will land at the average of reader and model', 'Nothing can ever be inferred from a metric published by a vendor'], correct: 1 },
    { q: 'Extending a tool validated as a reader aid into autonomous reading is best described as:', options: ['A new intended use requiring new evidence and regulatory standing', 'A configuration change to make once local accuracy has been confirmed', 'An efficiency decision that sits with the clinical lead alone', 'The same thing as raising the confidence threshold a little'], correct: 0 },
  ],
};
