...
const adM2: CourseModule = {
  ...
  quiz: [
    { q: 'Why does grouped-query attention (GQA) reduce the cost of long-context inference?', options: ['It skips attention for distant tokens', 'It quantises the attention weights to 4-bit', 'Multiple query heads share fewer key/value heads, shrinking the KV cache', 'It removes positional encoding overhead'], correct: 2 },
    { q: 'In a mixture-of-experts model, what does the "active parameter" count refer to?', options: ['The parameters actually used to process each token, selected by the router', 'The total parameters stored in GPU memory', 'The parameters updated during fine-tuning', 'The parameters in the attention layers only'], correct: 0 },
    { q: 'A model trained with RLHF starts producing longer, more flattering answers that raters score highly but users find less accurate. This is best described as:', options: ['Catastrophic forgetting', 'A tokenisation artefact', 'Retrieval failure', 'Reward hacking — the model exploiting flaws in the reward proxy'], correct: 3 },
    { q: 'How does byte-pair encoding (BPE) build its vocabulary?', options: ['By storing every dictionary word as one token', 'By iteratively merging the most frequent adjacent pairs, starting from raw bytes', 'By splitting text at whitespace and punctuation', 'By training a neural network to segment words'], correct: 1 },
    { q: 'What problem does PagedAttention (vLLM) primarily solve?', options: ['KV cache memory fragmentation, by allocating cache in small on-demand pages', 'Slow prompt prefill on long documents', 'Attention degradation in the middle of long contexts', 'Quantisation error in 4-bit models'], correct: 0 },
    { q: 'Why do hallucinations cluster on rare, specific facts like citations and API signatures?', options: ['Those tokens are filtered out of training data', 'The context window is too small to hold them', 'The model has weak memorised signal for rare facts and interpolates plausible-looking patterns instead', 'Safety filters block the correct answers'], correct: 2 },
    { q: 'Your RAG system\'s recall@10 on a golden query set is 45%. What does this tell you?', options: ['The LLM is hallucinating on half the answers', 'Over half the relevant passages never reach the model — no prompt change can recover them', 'The reranker is misordering results', 'The context window is too small'], correct: 1 },
    { q: 'What makes QLoRA cheaper to run than standard LoRA fine-tuning?', options: ['It trains fewer adapter layers', 'It uses a smaller training dataset', 'It skips the evaluation step', 'It quantises the frozen base model to 4-bit while training adapters in higher precision'], correct: 3 },
  ],
};
