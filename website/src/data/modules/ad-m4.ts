  ...
  quiz: [
    {
      ...
      options: [
        ...
        'More thinking improves every task proportionally to the tokens spent, so the reasoning budget should always be maxed out',
        'Test-time compute lowers serving cost by letting a smaller model stand in for a larger one',
        'Thinking tokens make responses arrive faster but measurably reduce the quality of the final answer',
      ],
      correct: 0,
    },
    {
      ...
      options: [
        'A frontier reasoning model on every ticket at the maximum thinking budget',
        'A reasoning model for every request, with aggressive prompt caching to hold the cost down',
        'A fast model for the bulk of traffic, escalating to reasoning only for hard or failed cases',
        'A hand-written rule engine only — LLMs cannot meet latency budgets at this volume',
      ],
      correct: 2,
    },
    {
      ...
      options: [
        'Its text tokeniser vocabulary is too small to cover the characters that appear in dense documents',
        'It generates with flow matching rather than diffusion, which handles fine detail poorly',
        'Its context window is too short to hold an image alongside the caption prompt',
    ...
    },
    {
      ...
      options: [
        'It generates directly in pixel space rather than in a compressed latent space, giving higher-fidelity output',
        'It learns near-straight noise-to-data transport, enabling fewer sampling steps and more stable training',
        'It removes the need for text conditioning by inferring the prompt from the noise schedule',
        'It replaces the transformer backbone with a recurrent network that scales linearly in resolution',
      ],
      correct: 1,
    },
    {
      ...
      options: [
        'Reward models grow too expensive to train once the base model passes a certain capability level',
        'Human raters will not read outputs long enough to contain expert-level reasoning, so coverage collapses',
        'Human judgement is the reward signal, and raters cannot reliably spot subtle flaws beyond their own expertise',
        'RLHF stops working above a fixed parameter count, at which point the policy gradient becomes numerically unstable',
      ],
      correct: 2,
    },
    {
      ...
      options: [
        ...
        'KV caching of intermediate attention states',
        'Speculative decoding with a draft model',
        'Low-rank adaptation (LoRA) of attention weights',
      ],
      correct: 0,
    },
    {
      ...
      options: [
        'The 2023 AI Executive Order remains in force as the binding federal framework for AI oversight',
        'The 2023 Executive Order was rescinded in January 2025, leaving a patchwork of state laws and sectoral agency enforcement',
        'A comprehensive federal AI act now mirrors the EU AI Act tier for tier, pre-empting conflicting state legislation',
        'Federal law now pre-empts the states, barring them from legislating on AI independently',
      ],
      correct: 1,
    },
    {
      ...
      options: [
        'Agents are being rolled back in favour of simpler chatbots, which teams have found more reliable in production',
        'Agents are converging on voice as the primary interface, with text becoming a fallback',
        'Agents are moving toward per-step human confirmation, with an explicit approval gate on every tool call',
