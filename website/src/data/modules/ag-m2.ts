...
const agM2: CourseModule = {
  ...
  quiz: [
    { q: 'What is the practical benefit of treating context assembly as a pure function of run state?', options: ['It removes the need for a hand-written system prompt, since the run state fully determines what the model is told', 'It can be tested and versioned without running the agent, and forces explicit decisions about what to leave out', 'It guarantees the context never overflows, because assembly is bounded by the size of the run state', 'It eliminates the need for retrieval, since everything the model needs is already held in state'], correct: 1 },
    { q: 'A team measures realised token share per context section for the first time. Which section most commonly turns out larger than expected?', options: ['The tool definitions, which are re-sent on every step regardless of use', 'The system instructions, which grow with every edge case the team has patched', 'The current observation, since raw tool output is rarely truncated before it lands', 'The scratchpad, which accumulates the model\'s own notes across every step of the run'], correct: 0 },
    { q: 'Where should a constraint that must hold for the entire run be placed in a long assembled context?', options: ['Once in the middle of the context, where it sits surrounded by the material it governs', 'Only in the opening message, since restating it wastes tokens and risks self-contradiction', 'Stated at the top and restated near the end of each step\'s context, rendered from run state', 'In the tool descriptions, so it is enforced at the point where the action is chosen'], correct: 2 },
    { q: 'Delimiting and labelling retrieved content as external material achieves what, precisely?', options: ['It prevents the model from acting on any instruction embedded in that content', 'It removes the need for egress controls, since content that cannot issue instructions cannot direct an outbound call', 'It marks the content as exempt from compaction, so retrieved passages survive intact to the end of a long run', 'It reduces accidental instruction-following, but is not a boundary against a determined embedded instruction'], correct: 3 },
    { q: 'Why is a structured extraction preferable to a free-text summary when compacting a long run?', options: ['It uses fewer tokens in every case, because structure removes the redundancy of prose', 'It can be validated and diffed, and makes a dropped constraint structurally visible', 'It allows the run to skip the compaction model call, since extraction can be done with a parser', 'Free-text summaries cannot be stored or retrieved reliably once the run has moved on'], correct: 1 },
    { q: 'Which material is most likely to be lost by compaction and cause a failure later?', options: ['Verbose tool output from the early exploratory steps of the run', 'Material stated once early and relied on late, such as an agreed exception or an identifier mentioned in passing', 'The most recent observation, which is summarised along with everything else', 'The tool definitions, which the compactor rewrites in condensed form'], correct: 1 },
    { q: 'An agent stores memories freely at any step with no schema. What is the characteristic result?', options: ['The store fills with transient details recorded as durable facts, and contradictions accumulate unresolved', 'Retrieval latency becomes the dominant bottleneck as the store grows past what a single query can usefully rank', 'Writes begin to fail, because the store has no schema against which to validate an entry before accepting it', 'The context window overflows on the very first step of the next run'], correct: 0 },
    { q: 'Which change most reliably destroys prompt cache hit rate in an agent loop?', options: ['Appending the newest tool result at the end of the assembled context', 'Keeping the tool definitions in a fixed order at the top of every request', 'Interpolating a timestamp or session identifier into the system instructions', 'Compacting the conversation history once the run passes its context threshold'], correct: 2 },
  ],
};
