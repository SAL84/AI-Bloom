...
const m5: CourseModule = {
  ...
  quiz: [
    { q: 'The dominant practical prompt injection threat in enterprise is:', options: ['Indirect injection via retrieved data (documents, web pages, tool outputs)', 'Direct jailbreaks of public chatbots', 'Physical access attacks', 'Password spraying'], correct: 0 },
    { q: 'The "lethal trifecta" that turns prompt injection into data exfiltration is:', options: ['Phishing, malware, and ransomware', 'Three failed login attempts', 'Private data access + exposure to untrusted content + an outbound channel', 'Jailbreak, poisoning, and evasion'], correct: 2 },
    { q: 'A "confused deputy" attack on an agent means:', options: ['The agent crashes', 'The model is fine-tuned poorly', 'The agent refuses to answer', 'The agent uses its legitimate privileges to act on the attacker behalf'], correct: 3 },
    ...
    { q: 'Why is persistent agent memory a distinct attack surface?', options: ['A poisoned instruction stored in memory re-executes across future sessions — a one-shot injection becomes a standing compromise', 'It uses too much storage', 'Memory is always encrypted', 'It slows the agent down'], correct: 0 },
    { q: 'In the OWASP Top 10 for LLM Applications, "excessive agency" refers to:', options: ['An agent that responds too quickly', 'Using more than one AI vendor', 'High API costs', 'Too much autonomy, too many permissions, and too little oversight for an agent'], correct: 3 },
    { q: 'MITRE ATLAS is best described as:', options: ['An EU regulation', 'A matrix of adversary tactics and techniques targeting AI systems — the AI counterpart to ATT&CK', 'A model benchmark suite', 'A Google product for agent governance'], correct: 1 },
    { q: 'The "asymmetry argument" for AI in security says:', options: ['AI only helps attackers', 'AI makes attack and defense identical', 'AI favors attackers at the entry point but favors defenders at scale — the question is who captures their advantage faster', 'Defenders should avoid AI until it is regulated'], correct: 2 },
