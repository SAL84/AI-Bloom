  ...
  lessons: [
    {
      ...
      slides: [
        ...
        { heading: 'The Three-Layer Model: Know Which Layer the Customer Is Buying', body: 'The Google Cloud security stack organizes into three layers — and misreading which layer a customer needs is the most common cause of misaligned deals. (1) Agents layer — what does the work: SecOps TIN/Hunting/Detection Engineering/Remediation agents, Google Threat Intelligence, Gemini Enterprise custom agents. (2) Governance layer — the strategic moat: Agent Identity (SPIFFE, GA), Agent Gateway (MCP policy enforcement, Preview), Agent Registry (catalog and versioning). (3) Protocol and data layer — the bus: MCP servers (SecOps GA, GTI, Workspace), Model Armor (prompt injection and DLP guardrails), Cloud Logging and Trace (immutable audit for every agent action). Entry question for every deal: "Which layer are you buying today, and which are you designing for?" A customer buying at layer 1 without layer 2 governance is building a security debt.' }
      ]
    },
    {
      id: 'm6l2',
    ...
    },
    {
      ...
      slides: [
        ...
        { heading: 'Wiz AI-Application Protection Platform (AI-APP)', body: 'The AI-specific layer of Wiz. Embeds AI security directly into developer workflows: real-time vulnerability scanning, AI-generated code security, dynamic AI Bill of Materials (AIBOM), automated remediation. Integrates with IDEs, Lovable, and version control. This is a relatively new capability area where Google has a meaningful lead — most competitors do not have a comparable AI-specific application security story.' },
        { heading: 'AI Supply Chain Risks and AppSec', body: 'A growing customer concern that most SE conversations have not yet caught up to: AI supply chain attacks. Vectors include malicious or backdoored model weights on public repositories (Hugging Face etc.), poisoned Python packages targeting AI development environments, and AI-generated code that embeds vulnerabilities in developer workflows at scale. The Wiz AI-APP AIBOM addresses this — it inventories every AI model, library, and dependency in the environment with version and provenance tracking, flagging untrusted sources. Discovery question: "Do you have visibility into which AI models and packages your developers are pulling into production, and where they came from?" Most development teams do not.' },
        { heading: 'The AppSec Developer Workflow', body: 'Wiz AI-APP embeds directly into the developer workflow rather than operating as a separate security tool: (1) IDE integration — real-time security feedback on AI-generated code as the developer writes it, before it reaches review. (2) Pull request scanning — automated security checks on every commit, with remediation suggestions that developers can apply in one click. (3) Runtime protection — monitors deployed AI workloads for anomalous inference patterns, unusual API call volumes, and model extraction attempts. (4) AIBOM generation — builds and maintains the AI Bill of Materials automatically as dependencies change. The positioning angle: most application security tools were designed before AI-generated code existed at scale. Wiz AI-APP is designed for the world where 30-50% of committed code is AI-generated.' },
