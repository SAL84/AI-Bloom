...
const agM1: CourseModule = {
  ...
  quiz: [
    { q: 'Under the working definition of a step, how should a single model response containing four parallel tool calls be counted?', options: ['As four steps, since the count should track how much work the tools actually performed', 'As one step — the model call plus the tool executions it requested and their observations', 'As zero steps until every tool has returned, because a step is only complete once its observations are in context', 'As one step per tool that succeeded, since a failed call advances nothing'], correct: 1 },
    { q: 'Why is a termination condition expressed only in the system prompt considered a defect?', options: ['Natural language cannot express a stopping condition precisely enough for a model to act on', 'It spends context tokens on every step that would be better given to tool definitions and observations', 'It makes control flow a probabilistic decision, so the run may stop early, never stop, or loop re-verifying', 'It suppresses tool calling, because the model reads the stop instruction as the higher-priority directive'], correct: 2 },
    { q: 'What is the main advantage of ending a run with an explicit terminal tool call rather than detecting finality in prose?', options: ['It reduces the cost of the final model call, since a tool call is cheaper than a prose summary', 'It guarantees the task was completed correctly, because the runtime validates the terminal call against the goal', 'It lets the model skip the verification step, since the terminal call itself certifies the work', 'Completion arrives as a structured artefact, distinguishable from giving up and loggable as a discrete event'], correct: 3 },
    { q: 'An agent has a twenty-step ceiling and stays within it, yet a run costs far more than expected. What is the most likely cause?', options: ['Each step carries a large and growing context that is re-sent on every model call', 'The step counter is broken and the ceiling is not being enforced', 'The model is sampled at too high a temperature, so it wanders into expensive detours', 'Tool executions are billed separately from steps and are not counted against the ceiling'], correct: 0 },
    { q: 'A parent agent has a remaining budget of ten steps and spawns three subagents. What is the correct budgeting behaviour?', options: ['Each subagent receives a fresh ten-step budget, since its work is independent', 'Subagents are exempt from step budgets because the parent step that spawned them is already counted', 'Subagents draw from the parent\'s remaining allowance, or delegation multiplies the effective ceiling', 'The parent\'s budget is suspended while subagents run and resumes when they report back'], correct: 2 },
    { q: 'Which failure is described as quieter than a runaway loop and at least as common?', options: ['Context exhaustion mid-run, which truncates the early instructions without warning', 'Premature stop — success declared on partial work, with a cheap run and a plausible report', 'Rate limiting from an external tool, which stalls the run part-way with no result', 'A schema validation error on a tool argument that the model repeats verbatim on every retry'], correct: 1 },
    { q: 'A stuck-agent detector fires after three identical tool fingerprints. Which response is least likely to help?', options: ['Injecting an observation naming the repetition and instructing a different approach', 'Removing the repeated tool from the available set so the next step must take another route', 'Terminating the run and escalating to a human with the full run state attached', 'Raising the step budget so the agent has more room to work through it'], correct: 3 },
    { q: 'What is the argument for shipping an agent first and then migrating parts of it into deterministic code?', options: ['Deterministic code is always cheaper to write and maintain than an equivalent prompt', 'It removes the need for tracing, since the migrated paths are already fully specified in code and behave deterministically', 'Traces reveal the dominant trajectories, which become testable code while the agent handles the hard tail', 'An agent cannot be evaluated meaningfully until its dominant paths have been replaced'], correct: 2 },
  ],
};
