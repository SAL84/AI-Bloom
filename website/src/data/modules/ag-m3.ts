...
const agM3: CourseModule = {
  ...
  quiz: [
    { q: 'Why is a tool with a vague description described as worse than not having the tool at all?', options: ['It consumes more context tokens than a precise description would, on every step of the run', 'It produces confident misuse that looks like a reasoning failure, whereas a missing tool fails cleanly', 'Its arguments cannot be validated against a schema, so malformed calls reach the implementation unchecked', 'It defeats prompt caching, because the model rewrites the vague description into its own reasoning on each step'], correct: 1 },
    { q: 'Two registered tools could each plausibly serve the same request. What is the likely consequence?', options: ['The model calls both and reconciles the results, which is slower but more reliable', 'Selection improves, because the model can pick whichever tool suits the specific request better', 'Inconsistent selection, because the discrimination has no basis the model can apply reliably', 'The runtime resolves the ambiguity by picking whichever of the two tools is cheaper to execute'], correct: 2 },
    { q: 'A tool returns an empty result set and, on failure, returns nothing. What problem does this create?', options: ['Success and failure look identical, so the model cannot tell "nothing matched" from "the query failed"', 'The context window overflows once empty results accumulate across many steps', 'The tool\'s results cannot be cached, because an empty response has no key to store against', 'Arguments can no longer be validated, since the schema has no case for the empty response'], correct: 0 },
    { q: 'Which property should a terminal error message make explicit?', options: ['The internal stack trace, so the agent can reason about where the call actually broke', 'The name of the downstream service that failed, so the agent can retry against an alternative', 'The latency of the failed call, so the agent can judge whether a retry is worth the budget', 'That retrying will not help, so the agent takes another route or escalates'], correct: 3 },
    { q: 'Why must tools with side effects be idempotent in an agent system?', options: ['Because the runtime deduplicates identical tool calls and rejects the second one outright', 'Because agents retry non-deterministically, so at-least-once execution is the honest assumption', 'Because idempotent tools return a cached result rather than re-executing, which cuts the cost of a long run', 'Because approval gates can only be applied to tools that are safe to execute repeatedly'], correct: 1 },
    { q: 'A tool creates a record, fails to send the notification, and reports a generic failure. What is the characteristic outcome?', options: ['The run terminates safely, since a reported failure means no side effects were committed', 'The model correctly infers the partial state and sends the missing notification on its own', 'The model retries and creates a duplicate record, having no way to tell partial failure from total', 'The runtime rolls the record creation back automatically once the tool reports a failure, leaving no trace'], correct: 2 },
    { q: 'Where is model self-critique least effective?', options: ['Checking written output against an explicit rubric supplied alongside it in the critique prompt', 'Spotting an unhandled edge case in code the model itself generated a step earlier', 'Noticing that one part of a multi-part question was left unanswered in the draft', 'Catching errors where the check shares the misunderstanding that produced the error'], correct: 3 },
    { q: 'What does adding a subagent reliably provide?', options: ['A separate context window, so exploration does not pollute the parent context', 'Higher reasoning quality on the same task, because the subagent starts from a clean slate', 'Lower total token cost per run, since the parent no longer carries the exploration', 'Deterministic behaviour on the delegated subtask, because its scope is narrow and fixed'], correct: 0 },
  ],
};
