...
const hcM1: CourseModule = {
  ...
  quiz: [
    { q: 'This course describes what a category of AI tool can do. What does that description authorise?', options: ['Switching the capability on in your own clinic\'s workflow next week', 'Setting the decision threshold your own service will act on, using the material here', 'Nothing clinical — local validation and governance sign-off still apply', 'Treating it as clearance already granted to every product in that category'], correct: 2 },
    { q: 'Which zone currently has the clearest and most consistent benefit case?', options: ['Autonomous diagnosis with no clinician review of the result', 'Fully automated treatment planning for complex cases', 'Predicting outcomes across a whole patient population', 'Reducing documentation and administrative burden'], correct: 3 },
    { q: 'Why does a tool that never makes a clinical claim still carry clinical risk?', options: ['Its output enters the record and downstream decisions rely on it', 'All software used in a hospital is automatically regulated as a medical device', 'Administrative tools are exempt from any testing before they go live', 'Clinicians do not read generated notes'], correct: 0 },
    { q: 'An ambient scribe produces a fluent, well-structured note. What is the most important residual risk?', options: ['The note will run longer than the record template comfortably allows', 'Omission or invention in a note that reads as correct and gets signed', 'The tool cannot separate two speakers who talk over each other', 'Formatting drifts between specialties across the same clinic'], correct: 1 },
    { q: 'A deterioration prediction model is accurate on retrospective data. What still has to be shown before it is useful?', options: ['That it runs fast enough to be useful at the bedside in real time', 'That the vendor is financially stable enough to keep supporting it', 'That it uses a model architecture no older than the current generation', 'That acting on the prediction is feasible and improves outcomes'], correct: 3 },
    { q: 'What most often prevents a successful pilot from reaching production?', options: ['Integration, workflow ownership, and maintenance were never resourced', 'Regulators step in and block the deployment', 'Model accuracy collapses without warning', 'Clinicians on the ward refuse to use any new technology'], correct: 0 },
    { q: 'A patient-facing symptom tool differs from clinician-facing decision support mainly because:', options: ['It runs on a different kind of model underneath', 'It handles data that is less sensitive than a clinical record', 'There is no clinician between the output and the person acting on it', 'It sits outside the reach of the data protection rules that cover clinical systems'], correct: 2 },
  ],
};
