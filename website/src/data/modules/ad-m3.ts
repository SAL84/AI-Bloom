...
const adM3: CourseModule = {
  ...
  title: 'Agentic AI',
  icon: 'layers',
  summary: 'Agent architecture patterns, orchestration frameworks, MCP and A2A protocols, the agentic attack surface, production reliability, context engineering, and evals.',
  lessons: [
    {
      id: 'ad3l1',
  ...
  quiz: [
    { q: 'What distinguishes a true agent from a workflow in the workflow-to-agent spectrum?', options: ['Agents use more powerful models than workflows', 'Agents always involve multiple cooperating models', 'In an agent, the model itself directs control flow — which tools to call, in what order, and when to stop', 'Workflows cannot call external tools'], correct: 2 },
    { q: 'What is the strongest argument of the build-on-raw-APIs school of agent development?', options: ['The core loop is small, and framework abstractions add opacity exactly where debugging needs transparency', 'Raw APIs are always cheaper per token than frameworks', 'Frameworks cannot support tool calling', 'Raw APIs remove the need for retries and state management'], correct: 0 },
    { q: 'In MCP, what is the key difference between tools and resources?', options: ['Tools are remote and resources are local', 'Tools are model-controlled actions; resources are application-controlled data the host places into context', 'Resources are executable and tools are read-only', 'Tools use OAuth while resources do not'], correct: 1 },
    { q: 'What is a "rug pull" in the context of MCP tool poisoning?', options: ['An agent exceeding its token budget mid-task', 'A user revoking OAuth consent during a session', 'A judge model changing its scores between runs', 'A trusted server changing its tool definitions to malicious ones after gaining adoption'], correct: 3 },
    { q: 'Which combination of agent properties forms the classic exfiltration danger pattern?', options: ['Access to private data, exposure to untrusted content, and ability to communicate externally', 'Long context, multiple subagents, and streaming output', 'Persistent memory, prompt caching, and parallel tool calls', 'High temperature, large toolset, and long-running sessions'], correct: 0 },
    { q: 'Why should agent regression tests assert on pass rates across multiple runs rather than on a single transcript?', options: ['Because single runs are too expensive to record', 'Because agents are non-deterministic — the same scenario can legitimately produce different trajectories', 'Because providers forbid storing individual transcripts', 'Because pass rates eliminate the need for tracing'], correct: 1 },
    { q: 'For prompt caching to work in an agent loop, how should the context be laid out?', options: ['Volatile content first so it is always fresh', 'Tool results before tool definitions', 'Stable content (system prompt, tool definitions) as a fixed prefix, volatile content appended after', 'Alphabetically, so ordering is deterministic'], correct: 2 },
    { q: 'What is position bias in LLM-as-judge evaluation, and what is the standard mitigation?', options: ['Judges favour longer answers; mitigate by truncating outputs', 'Judges favour their own model family; mitigate by fine-tuning the judge', 'Judges favour answers containing keywords; mitigate with stop words', 'Judges favour one side in pairwise comparisons; mitigate by running both orderings and reconciling'], correct: 3 },
  ],
};
